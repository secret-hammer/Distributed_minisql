...
class MasterServer(object):
        ...
        def Region_watcher(children):
            ...
            for host, state in self.RegionServer.items():
                if state == 0 and host not in host_list:
                    ...
                    self.RegionServer[host] = -1  ###存疑，是-1还是1
    ...
    def find_best_region_server(self):
        temp_RegionServer = ""
        temp_min_region_server = 10000 #假设现在连接最多的region server有10000个连接
        for host in list(self.RegionServer.keys()):
            if((self.RegionServer[host] < temp_min_region_server) and (self.RegionServer[host] >= 0)):
                temp_RegionServer = host
                temp_min_region_server = self.RegionServer[host]
        master_region_host, client_region_host = temp_RegionServer.split(';')
        return client_region_host
